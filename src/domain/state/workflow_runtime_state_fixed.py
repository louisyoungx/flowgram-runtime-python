            ...
            for key, value_config in input_values.items():
                ...
                if value_type == "constant":
                    ...
                    if len(value_content) >= 2:
                        ...
                        elif node_id == "start_0" and len(value_content) > 2:
                            # Try to get the parent object
                            parent_key = output_key
                            if self._variable_store.has_variable(parent_key):
                                parent_value = self._variable_store.get_variable(parent_key)
    ...
    def parse_ref(self, ref: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if self._variable_store.has_variable(variable_key, node_id=node_id):
            ...
            if value_type:
                ...
                if value_type == WorkflowVariableType.Array and isinstance(value, list) and len(value) > 0:
                    first_item = value[0]
