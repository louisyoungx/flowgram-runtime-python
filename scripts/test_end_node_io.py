...
async def test_end_node_io():
    ...
    end_node_snapshots = [s for s in context.snapshot_center.export_all() if s.node_id == "end_0"]
    if end_node_snapshots:
        end_snapshot = end_node_snapshots[0]
        logger.info(f"End node inputs: {end_snapshot.inputs}")
    else:
    ...
    if end_node_snapshots and context.io_center.outputs == end_snapshot.inputs:
        ...
        logger.error(f"End node inputs: {end_node_snapshots[0].inputs if end_node_snapshots else 'N/A'}")
