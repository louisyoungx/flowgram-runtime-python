"""
Test for condition node handling of array types and is_empty/is_not_empty operators.
"""
import os
import sys
import asyncio
            ...
            "blocks": [
                {
                    ...
                    "data": {
                        ...
                        "inputsValues": {
                            ...
                            "systemPrompt": {"type": "constant", "content": "用户会传入一个prompt，你需要将这个prompt扩写为一个完整的标题，例如输入露营，返回在瑞士阿尔卑斯山的露营之旅"},
...
class ConditionArrayTest(unittest.TestCase):
    ...
    async def test_empty_array_condition(self):
        ...
        print(f"Initial workflow status: {context.status_center.workflow.status}")
        ...
        print(f"Snapshots: {len(snapshots)} snapshots found")
        
        # Verify condition branch
        ...
        if condition_snapshots:
            ...
            branch = condition_snapshot.data.get("branch", "")
    ...
    async def test_complex_workflow_empty_array(self):
        ...
        try:
            ...
            result = await asyncio.wait_for(task.processing, timeout=5)
            print(f"Final workflow status: {context.status_center.workflow.status}")
            print(f"Result: {result}")
            ...
            if condition_snapshots:
                ...
                branch = condition_snapshot.data.get("branch", "")
                print(f"Condition branch: {branch}")
                self.assertEqual(branch, "if_OVHXT")
            else:
                print("No condition snapshot found")
        except asyncio.TimeoutError:
            print("Test timed out, but condition evaluation should have completed")
            
            # Verify condition branch even if task didn't complete
            snapshots = context.snapshot_center.export_all()
            condition_snapshots = [s for s in snapshots if s.node_id == "condition_Ts3zx"]
            if condition_snapshots:
                condition_snapshot = condition_snapshots[0]
    ...
    async def test_complex_workflow_non_empty_array(self):
        ...
        try:
            ...
            if condition_snapshots:
                ...
                branch = condition_snapshot.data.get("branch", "")
                print(f"Condition branch: {branch}")
                self.assertEqual(branch, "if_pTr8_")
            else:
                print("No condition snapshot found")
        except asyncio.TimeoutError:
            print("Test timed out, but condition evaluation should have completed")
